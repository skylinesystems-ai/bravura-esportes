...
const bootLines = [
  "INICIANDO SISTEMA...",
  "CONECTANDO À ARENA BRAVURA...",
  "CATÁLOGO ESPORTIVO CARREGADO...",
  "USUÁRIO DETECTADO...",
...
export default function IntroScreen({ onEnter }) {
  ...
  return (
    ...
    >
      <div className="absolute inset-0 bg-hud-grid bg-[length:54px_54px] opacity-20" />
      <div className="absolute inset-0 intro-energy" />
      <div className="scanner-line" />
      <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-bravura-blue to-transparent" />
      <div className="absolute inset-x-0 bottom-0 h-px bg-gradient-to-r from-transparent via-bravura-orange to-transparent" />

      {particles.map((particle) => (
        <motion.span
          key={particle.id}
          className="absolute h-1 w-1 rounded-full bg-bravura-blue shadow-[0_0_14px_rgba(0,217,255,.85)]"
      ...
      >
        <div className="mx-auto mb-8 flex h-56 w-56 items-center justify-center border border-bravura-orange/25 bg-bravura-graphite/55 shadow-neon backdrop-blur-xl sm:h-72 sm:w-72">
          <BrandLogo size="xl" />
        </div>

        <div className="mx-auto mb-7 max-w-xl border border-bravura-blue/20 bg-black/45 p-5 text-left font-display text-xs uppercase leading-7 tracking-[0.18em] text-bravura-ice shadow-cyan backdrop-blur-xl sm:text-sm">
          <div className="mb-3 flex items-center gap-2 text-bravura-green">
            <RadioTower size={16} />
            <span>Sistema Bravura Online</span>
          </div>
